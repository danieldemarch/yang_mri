...
class RecEnv(object):
    def __init__(self, args, split):
        ...
        self.action_size = self.dataset.image_shape[0]
        self.max_acquisition = args.max_acquisition
        self.center_acquisition = args.center_acquisition
    ...
    def reset(self, loop=True, init=False):
        ...
        try:
            ...
            pad = (self.x.shape[1]-self.center_acquisition+1) // 2
            self.m[:,pad:pad+self.center_acquisition] = 1.
    ...
    def step(self, action, prediction):
        empty = action == -1
        terminal = np.sum(self.m[:,:,0,0],axis=1) == self.max_acquisition
